...
class Simulator:
    ...
    def plot_results(self, q_des: np.array = None, dq_des: np.array = None):
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'logs/plots/position.png')
        plt.close()
        ...
        if q_des:
            plt.figure(figsize=(10, 6))
            for i in range(self.pos.shape[1]):
                plt.plot(self.times, q_des[i] - self.pos[:, i], label=f'Joint {i+1}')
            plt.xlabel('Time [s]')
            plt.ylabel('Joint Position error [rad]')
            plt.title('Joint Position error over Time')
            plt.legend()
            plt.grid(True)
            plt.savefig(f'logs/plots/position_error.png')
            plt.close()

        # Joint velocities plot
        plt.figure(figsize=(10, 6))
        for i in range(self.vel.shape[1]):
            plt.plot(self.times, self.vel[:, i], label=f'Joint {i+1}')
        plt.xlabel('Time [s]')
        plt.ylabel('Joint Velocities [rad/s]')
        plt.title('Joint Velocities over Time')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'logs/plots/velocities.png')
        plt.close()
        ...
        if dq_des:
            plt.figure(figsize=(10, 6))
            for i in range(self.vel.shape[1]):
                plt.plot(self.times, dq_des[i] - self.vel[:, i], label=f'Joint {i+1}')
            plt.xlabel('Time [s]')
            plt.ylabel('Joint Velocity error [rad]')
            plt.title('Joint Velocity error over Time')
            plt.legend()
            plt.grid(True)
            plt.savefig(f'logs/plots/velocity_error.png')
            plt.close()
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'logs/plots/control_signals.png')
        plt.close()